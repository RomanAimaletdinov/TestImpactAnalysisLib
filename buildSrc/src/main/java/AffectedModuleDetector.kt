...
private class AcceptAll(
    ...
    override fun shouldInclude(project: Project): Boolean {
        println("ROMAN: shouldInclude, AcceptAll")
...
class AffectedModuleDetectorImpl constructor(
    ...
    override fun shouldInclude(project: Project): Boolean {
        println("ROMAN: shouldInclude -> isRoot: ${project.isRoot}, affectedProjects.contains(project): ${affectedProjects.contains(project)}, isProjectProvided2(project): ${isProjectProvided2(project)}")
        println("ROMAN: affectedProjects -> $affectedProjects")
        return (
            (project.isRoot || (
    ...
    override fun isProjectProvided2(project: Project): Boolean {
        if(modules == null ) {
            println("isProjectProvided2: modules == null")
            return true
        }
        println("isProjectProvided2: modules -> $modules")
    ...
    private fun findAffectedProjects(): Set<Project> {
        ...
        if (projectSubset == ProjectSubset.CHANGED_PROJECTS) {
            println("ROMAN: Вернул changedProjects вместо анализа")
            return changedProjects
        }
        ...
        if (changedProjects.isEmpty() && unknownFiles.isEmpty()) {
            println("ROMAN: buildAll ставим тру, тк нет измененных файлов")
            buildAll = true
        }
        logger?.info(
